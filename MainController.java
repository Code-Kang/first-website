...
public class MainController {
    ...
    public String userLogin(User user) throws Exception {
        ...
        user.setUsPassword(encPasswd);
        User info = adminService.userLogin(user);
        User failcnt = adminService.failCount(user);

        if (info == null) {
            adminService.updateloginFailCount(user);
            jsonObject.put("flg", "N");
            jsonObject.put("msg","로그인 실패");
            jsonObject.put("failcnt",failcnt.getLoginFailCount());
...
//            jsonObject.put("id", info.getUsId());
//            jsonObject.put("tel", info.getUsTel());
            jsonObject.put("msg", "로그인 성공");
            if (failcnt.getLoginFailCount() <= 5){
...
//        adminSession.login(loginAdmin);
    ...
    public String idCheck(User user) throws Exception {
        ...
        JSONObject jsonObject = new JSONObject();
        user.setUsId(chId);
        User check = adminService.idCheck(user);

        if (check == null) {
            jsonObject.put("flg", "Y");
            jsonObject.put("msg", "사용 가능한 아이디 입니다.");
            return jsonObject.toString();
        }else if (chId.equals(check.getUsId())) {
            jsonObject.put("flg", "N");
...
//    @ResponseBody
//    public String insertUserAdd(User user) throws Exception {
//
//        String encPasswd = CryptoUtils.encodeSHA512(passwd);
////        User info = adminService.insertUserAdd(user);
//        return info.setUsId();
//    }
